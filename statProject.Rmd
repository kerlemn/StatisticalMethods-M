...
# Generalized Additive Models (GAM) on Original Dataset
...
inter_model <- gam(Personal.Loan ~ s(Income) + s(CCAvg) +
                    Family + Education + Securities.Account + 
                    CD.Account + Online + CreditCard + Income:Family + Income:Education , data = data, family = binomial)
summary(inter_model)
```

Then we plots the residuals of inter_model to assess the model's fit.
```{r}
plot(inter_model, residuals=TRUE, pch=19, pages=1)
```

To determine the most effective model, we calculated and compared the Akaike Information Criterion (AIC) values of full_model, model, and inter_model, alongside their deviance residuals. 
```{r}
res_dev= c(full_model$deviance, model$deviance, inter_model$deviance)
as.matrix(AIC(full_model, model, inter_model) %>% mutate(res_dev=res_dev))
```
...
```{r}
formula <- Personal.Loan ~ s(Income) + s(CCAvg) + Family + Education + Securities.Account + CD.Account + Online + CreditCard + Income:Family + Income:Education
...
cv <- function(data, k = 10) {
  # Creating k folds of equal sizes
  folds <- createFolds(data$Personal.Loan, k = k)
  
  # Initialize matrices to store evaluation metrics for each model
  metrics_matrix <- matrix(0, nrow = 3, ncol = 4, dimnames = list(c("Full Model", "GAM Model", "Interaction Model"), c("Accuracy", "TPR", "TNR", "AUC")))
  
  # Define the formula for the GAM model
  formula <- Personal.Loan ~ s(Age) + s(Income) + s(CCAvg) + s(Mortgage) +
    Family + Education + Securities.Account + 
  ...
  for (i in 1:k) {
    ...
    model_formula <- Personal.Loan ~ s(Income) + s(CCAvg) +
      Family + Education + Securities.Account + 
      CD.Account + Online + CreditCard
    model <- gam(model_formula, data = train, family = binomial)
    
    # Training the Interaction Model
    inter_formula <- Personal.Loan ~ s(Income) + s(CCAvg) +
      Family + Education + Securities.Account + 
      CD.Account + Online + CreditCard + s(Income, by = Family) + s(Income, by = Education)
    inter_model <- gam(inter_formula, data = train, family = binomial)
    
    # Making predictions on the test set for each model
    predicted_full <- predict(full_model, newdata = test, type = "response")
    predicted_model <- predict(model, newdata = test, type = "response")
    predicted_inter <- predict(inter_model, newdata = test, type = "response")
    
    # Calculating evaluation metrics for each model
    cm_full <- confusionMatrix(as.factor(ifelse(predicted_full > 0.5, 1, 0)), as.factor(test$Personal.Loan))
    cm_model <- confusionMatrix(as.factor(ifelse(predicted_model > 0.5, 1, 0)), as.factor(test$Personal.Loan))
    cm_inter <- confusionMatrix(as.factor(ifelse(predicted_inter > 0.5, 1, 0)), as.factor(test$Personal.Loan))
    
    # Store evaluation metrics for each model in the matrix
    metrics_matrix["Full Model", ] <- metrics_matrix["Full Model", ] + c(cm_full$overall['Accuracy'], cm_full$table['1','1'] / sum(cm_full$table['1',]), cm_full$table['0','0'] / sum(cm_full$table['0',]), roc(test$Personal.Loan, predicted_full)$auc)
    metrics_matrix["GAM Model", ] <- metrics_matrix["GAM Model", ] + c(cm_model$overall['Accuracy'], cm_model$table['1','1'] / sum(cm_model$table['1',]), cm_model$table['0','0'] / sum(cm_model$table['0',]), roc(test$Personal.Loan, predicted_model)$auc)
    metrics_matrix["Interaction Model", ] <- metrics_matrix["Interaction Model", ] + c(cm_inter$overall['Accuracy'], cm_inter$table['1','1'] / sum(cm_inter$table['1',]), cm_inter$table['0','0'] / sum(cm_inter$table['0',]), roc(test$Personal.Loan, predicted_inter)$auc)
  }
  
  # Calculate the mean of evaluation metrics for each model
  metrics_matrix <- metrics_matrix / k
  
  # Return the matrix of mean evaluation metrics
  return(metrics_matrix)
}

# Using the cv function
cv_results_means <- cv(data, k = 10)
...
full_model_oversampled <- gam(Personal.Loan ~ s(Age) + s(Income) + s(CCAvg) + s(Mortgage) +
                    Family + Education + Securities.Account + 
                    CD.Account + Online + CreditCard , data = data_oversampled2, family = binomial)
...
inter_model_oversampled <- gam(Personal.Loan ~ s(Age) + s(Income) + s(CCAvg) + Mortgage +
                    Family + Education + Securities.Account + 
                    CD.Account + Online + CreditCard + Income:Family + Income:Education , data = data_oversampled2, family = binomial)
...
cv <- function(data, k = 10) {
  ...
  for (i in 1:k) {
    ...
    model_formula <- Personal.Loan ~ s(Income) + s(CCAvg) +
      Family + Education + Securities.Account + 
      CD.Account + Online + CreditCard
    model_oversampled <- gam(model_formula, data = train, family = binomial)
    
    inter_formula <- Personal.Loan ~ s(Income) + s(CCAvg) +
      Family + Education + Securities.Account + 
      CD.Account + Online + CreditCard + s(Income, by = Family) + s(Income, by = Education)
    inter_model_oversampled <- gam(inter_formula, data = train, family = binomial)
    
    predicted_full <- predict(full_model_oversampled, newdata = test, type = "response")
    predicted_model <- predict(model_oversampled, newdata = test, type = "response")
    predicted_inter <- predict(inter_model_oversampled, newdata = test, type = "response")
...
cv_results_means <- cv(data_oversampled2, k = 10)
print(cv_results_means)

```

By using oversampling, we effectively addressed the issue of imbalanced classes, resulting in improved model performance compared to those trained on the original dataset. While the oversampled model had slightly lower accuracy than the initial models, it still showed strong predictive ability and well-balanced performance. Overall, the models trained on the oversampled data handled class imbalance well and remained competitive in predicting loan eligibility.

In conclusion, our GAM models demonstrate promising potential for predicting the probability of obtaining a personal loan based on the provided variables.
...
train_data_oversampled = minority_train_data[sample(1:nrow(minority_train_data), size = nrow(majority_train_data), replace = TRUE), ] %>%
...
Since with many data the Bayesian model is very similar to the classic glm one, we tried to balance the dataset through undersampling.
...
# Combine indices of minority and undersampled majority class
...
# Use the sampled indices to reorder the rows
...
```{r}
# Glm model to compare with the Bayesian
...
One of the initial challenges encountered in this modeling process is setting the prior distributions for the coefficients, it forced us to have an idea about the underlying system that produces the outcome variable Personal.Loan. 

Since we don't have domain expertise and no expert opinion available, we did some research and developed our own opinion on the way the variables may influence the Personal.Loan variable.
...
We can state that Age, Mortgage and Securities.Account are not significant.
...
Let's try to modify the model cutting some variables and modify the priors (Mortgage, Age, Securities.Account the coefficient estimate is very close to 0 and the C.I. is very small) and computing again the WAIC.
...
waic(model1)
```
It is interesting to note that we get a lower WAIC, then we choose to take the reduced model.
Let's assess it.
